...
class Tagger(abc.ABC):
    ...
    def __init__(
        ...
        self.classifiers_set: Set[str] = set()
    ...
    @classmethod
    def normalize(cls, word: str, lower=True) -> str:
        """default normalization"""
        word_form = word.lower() if lower else word
        return re.sub(r"^\W+|\W+$", "", word_form)
    ...
    def formality(
        ...
        for src, tgt, align in zip(src_doc, tgt_doc, align_doc):
            tags = []
            for word in tgt:
                norm_word = self.normalize(word.text, lower=False)
        ...
        for src, tgt, align, antecs in zip(src_doc, tgt_doc, align_doc, antecs_doc):
            ...
            for s, r in align.items():
                ...
                if (
                    ...
                    and src_pos[s] == "PRON"
                    and tgt_pos[r] == "PRON"
                    and self.normalize(tgt_text[r], lower=False)
                    in self.ambiguous_pronouns.get(self.normalize(src_text[s], lower=True), [])
                ):
                    tags[tgt_idx[r]] = True
            doc_tags.append(tags)
    ...
    def classifiers(
        ...
        """Detect and tag the classifiers in the doc"""
        doc_tags = []
        for src, tgt, align, antecs in zip(src_doc, tgt_doc, align_doc, antecs_doc):
            tags = [False] * len(tgt)
            for i, token in enumerate(tgt):  # Iterate over each token in the current sentence
                # Check if the token is a classifier
                if token.text in self.classifier_set:
                    tags[i] = True  # Mark the token as a classifier
            doc_tags.append(tags)
